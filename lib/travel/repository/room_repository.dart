...
class RoomRepository {
  ...
  Future<List<RoomModel>> getAllRoomByHotelIdRoomGuest(String hotelId,
      int guest, int room) async {
    try {
      var querySnapshot = await _firebaseFirestore
          .collection('room')
          .where('hotel', isEqualTo: hotelId)
  ...
  Future<RoomModel?> removeRoomById(String roomId, int maxGuest,
      int maxRoom) async {
    try {
      var roomDoc =
  ...
  Future<RoomModel?> addRoomById(String roomId, int maxGuest,
      int maxRoom) async {
    try {
      var roomDoc =
      await _firebaseFirestore.collection('room').doc(roomId).get();
